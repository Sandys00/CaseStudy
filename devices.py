...
class Device():
    ...
    def load_device_by_name(cls, device_name):
        ...
        if result:
            data = result[0]
            return data
        #cls(data["device_name"], data["managed_by_user_id"], data["end_of_life"], data["first_maintenance"], data["next_maintenance"], data["__maintenance_interval"], data["__maintenance_cost"], data["reservation"])
        else:
            return None
